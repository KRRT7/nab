...
| `--http-backend {urllib3,httpx,httpx2}` | `urllib3` | Pick the async transport for fetches. Layered, so it can also be set in an `nab.toml` or `NAB_HTTP_BACKEND`. `httpx` and `httpx2` need their extras (see [Install nab](../how-to/install.md)). |
